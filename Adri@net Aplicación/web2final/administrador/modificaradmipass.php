...
function comprobar() {
         var mirarLargo="no";
         var mirarIgual="no";
         var contra1 = document.formu.pass1
         var contra2 = document.formu.pass2
		 var espacios = false;
         var cont = 0;
  ...
  <table>
    <tr>
      <td>Modificacion de Datos</td>
	    ...
	    <tr>
      <td>Indique su antigua contraseña:</td>
      <td><input name="passw" type="password" id="pass"   required="required"/ ></td>
    </tr>

    <tr>
      <td>Indique la contraseña nueva:</td>
      <td><input name="password" type="password" id="password"   required="required"/ ></td>
    </tr>

    <tr>
      <td>Repita la contraseña nueva:</td>
      <td><input name="password1" type="password" id="password1"   required="required"/ ></td>
    </tr>
    <tr>
      <td><div align="right">
...
if(crypt($passw, $y['password']) != $y['password']){
echo '<script language="javascript">
alert ("La contraseña actual no coincide por favor verifique.");
window.location="modificarclientpass.php";
</script>';
}
else{

		$a="UPDATE cliente SET password='".$pass."' where cedula='".$cedula."'";
 		mysql_query($a, $db);
		$sql="INSERT INTO auditoria (id_auditoria, nom_client, accion, correo, hora, fecha , perfil) VALUES (null, '".$_SESSION['nombre']."', 'Modifico su contraseña','".$_SESSION['email']."', now() , now() , '".$_SESSION['perfil']."' )";
mysql_query($q, $db);
?>
